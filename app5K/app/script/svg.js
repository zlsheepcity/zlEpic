...
var pathdance_dancers = [
    {
        ...
        pathloop: [pathdance_library.arrow1, pathdance_library.arrow2],
        duration: 1000
    }
]
...
function PathdanceKapellmeister() {
    this.Dancing_Path = function(dna) {
        ...
        this.Start = function() {
            ...
            Pathdance.schedule.push(this.name);
            this.isReady = false;
            console.log('Start '+this.name);
            requestAnimationFrame(Pathdance.DrawRequest);
            return this;
        }
        this.Stop = function() {
            this.startTime  = false;
    ...
    this.DrawRequest = function(requestTime) {
        if (!Pathdance.schedule.length) return false;
        for ( var name in Pathdance.schedule )
            if ( Pathdance.DanceOperator('arrow', requestTime) === 'keep' )
                requestAnimationFrame(Pathdance.DrawRequest);
        //console.log('DrawRequest');
        //console.log(requestTime);
        //console.log(this);
        //console.log(Pathdance.schedule);
        
    }
    this.DanceOperator = function(name, requestTime) {
