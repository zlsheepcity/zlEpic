...
function initTheGame()
{
  ...
  TheGame.postScore = 0;
  TheGame.preScoreMax = 0;
  TheGame.preScoreCurrent = 0;
  Q.Feedback.hide();
  ...
  for ( i=0;i<TheGame.numberOfQuestions;i++) Q.ScoreBar.append(Q.tmpl.unset);
}
...
function loadDialog( dialogID )
{
  if ( dialogID )
  {
    if ( TheGame.preScoreCurrent > TheGame.preScoreMax ) TheGame.preScoreMax = TheGame.preScoreCurrent;
	TheGame.preScoreCurrent = 0;
	CD.DOM = $('#'+dialogID);  
    CD.currentLevel = 1;
...
function runDialogStep()
{
  ...
  Q.Output.append( CL.children('h2').clone() ); // .prepend('<span>'+CD.currentLevel+')</span> ')
  Q.Output.append( CL.children('div.filling').clone().delay(1000).fadeIn() ); //div "Filling out..."
...
function clickAnswer(e)
{
  ...
  CD.currentLevel++;
  if ( !LI.hasClass('correct') ) { finishDialog(false); return false; } //CD.noErrors = false;
  
  if ( CD.currentLevel > CD.maxLevel )  finishDialog(true);
  else                                  
  {
	if ( !TheGame.theMinimumGoalReached ) TheGame.preScoreCurrent++;
	runDialogStep();
  }
...
function finishDialog( theResultWasGood )
{
  ...
  if (theResultWasGood)
  {
	...
	Q.Output.append(H2);
    FEEDBACK.append('<div class="result good">Разговор с клиентом окончен, <br>все прошло отлично!</div>');
	
	var unlock = TheGame.isUnlockNow();
	if ( !unlock && TheGame.theMinimumGoalReached ) TheGame.postScore++;
  ...
  else
  {
	...
	Q.Output.append(H2);
    FEEDBACK.append('<div class="result bad">Разговор окончен, <br>т.к. в нем были допущены ошибки. Попробуй позвонить следующему клиенту.</div>');
  }
  ...
  {
    NEXT = $('<button class="y-button">Показать результат</button>');
...
{    
  ...
  if ( !TheGame.theMinimumGoalReached ) curScore = 0; // TheGame.preScoreMax < 5 ? TheGame.preScoreMax : 4;
	else if ( TheGame.dialoguesCompletedSuccessful == TheGame.dialoguesCompleted ) curScore = 10;
	else curScore = 5 + TheGame.postScore;
  ...
  */
  ...
  // calculate money
  var moneyEarned = 0;
  if ( curScore > 5 && curScore > TheGame.bestResult ) 
  {
	// check for update
	moneyEarned = ( curScore - TheGame.bestResult ) * 10;
  }
  else if ( TheGame.theMinGoalThisSession )
  {
	// money for unlock
	moneyEarned = 50;
  }
  ...
  // set type of feedback
  var feedbackType = 'looser';
  if ( TheGame.theMinGoalThisSession ) 							feedbackType = 'wasunlock';
  else if ( curScore > 5 && curScore > TheGame.bestResult )		feedbackType = 'wasupdate';
  else if ( curScore >= 5 )										feedbackType = 'badupdate';
  ...
  Q.Feedback.children('.content').children('.ifResultIsWorst').hide();
	...
	if ( curScore > TheGame.bestResult ) 	Q.Feedback.children('.content').children('.ifResultIsImproved').show();
	if ( curScore > 0 && curScore < 10 ) 	Q.Feedback.children('.content').children('.ifResultIsNotImproved').show();
	
	if ( curScore >= 10 && TheGame.bestResult >= 10 ) Q.Feedback.children('.content').children('.ifThisIsMaster').show();
	else if ( curScore >= 10 ) Q.Feedback.children('.content').children('.ifThisIsMasterNow').show();
	else if ( !TheGame.theMinimumGoalReached ) Q.Feedback.children('.content').children('.ifResultIsWorst').show();
  ...
  */
  ...
  var values =
  {
    ...
    CurResultProcent    : TheGame.dialoguesCompletedSuccessful > 0 ? (curScore * 10).toString() + '%' : '0%',
